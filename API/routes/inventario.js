...
const COLL = 'inventario';
const KEY = 'idInventario';
...
router.get('/:id', (req, res) => {
  ...
  if (!item) return res.status(404).json({ message: 'Inventario no encontrado' });
  res.json(item);
});
...
router.post('/', (req, res) => {
  ...
  if (data[COLL].some((x) => String(x[KEY]) === String(newItem[KEY])))
    return res.status(400).json({ message: 'Inventario ya existe' });
...
router.put('/:id', (req, res) => {
  ...
  if (idx === -1) return res.status(404).json({ message: 'Inventario no encontrado' });
