...
router.get('/:id', (req, res) => {
  ...
  const item = (data[COLL] || []).find((x) => x[KEY] === id || x.Id_Lote === id);
  if (!item) return res.status(404).json({ message: 'Lote no encontrado' });
  res.json(item);
});
...
router.post('/', (req, res) => {
  ...
  if (data[COLL].some((x) => x[KEY] === newItem[KEY]))
    return res.status(400).json({ message: 'Lote ya existe' });
...
router.put('/:id', (req, res) => {
  ...
  const id = req.params.id;
  data[COLL] = data[COLL] || [];
  const idx = data[COLL].findIndex((x) => x[KEY] === id || x.Id_Lote === id);
  if (idx === -1) return res.status(404).json({ message: 'Lote no encontrado' });
